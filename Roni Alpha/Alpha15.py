...
def plot_molecule(xyz_data, connections, element_data, atom_numbers, file_name, nav):
    ...
    def onpick(event):
        ...
        if len(nav.selected_atoms) == 2:
            ...
            print(midpoint, direction_vector, B1)
            print(type(B1), type(direction_vector), len(direction_vector), type(midpoint), len(midpoint))
            
            # Plot B1 vector in green
            ax.quiver(float(midpoint[0]), float(midpoint[1]), float(midpoint[2]), 
                      float(direction_vector[0]), float(direction_vector[1]), float(direction_vector[2]), 
                      length=B1, color='green', arrow_length_ratio=2)
            
            # Plot B5 vector in red
            ax.quiver(midpoint[0], midpoint[1], midpoint[2], 
                      direction_vector[0], direction_vector[1], direction_vector[2], 
                      length=B5, color='red', arrow_length_ratio=0.2)
